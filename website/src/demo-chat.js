(() => {
  ...
  launch.type = "button";
  launch.textContent = "Try the HVAC demo";
  root.appendChild(launch);

  const panel = document.createElement("div");
  panel.className = "br-panel";
  panel.innerHTML = '<div class="br-head">HVAC intake demo<div class="br-sub">Fictional business • no real service request</div></div><div class="br-log"></div><form class="br-form"><input class="br-input" autocomplete="off" maxlength="1600" placeholder="Type a message…"><button class="br-send" type="submit">Send</button></form><div class="br-note">Do not send passwords, Social Security numbers, or full payment-card numbers.</div>';
  ...
  launch.addEventListener("click", () => {
    ...
    if (panel.classList.contains("open")) {
      if (!log.childNodes.length) add("assistant", "This is a fictional HVAC demo. Use made-up details. What HVAC issue can I help capture?");
      input.focus();
    }
  });
